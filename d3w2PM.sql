...
-- Time Different with the data & time interval  
SELECT 
	id, 
	founded_at,
	CURRENT_DATE AS todays_date,
	CURRENT_DATE - founded_at AS founded_time_ago,
	founded_at + INTERVAL '10 DAY' AS plus_10_day_after_funded
FROM companies;
...
-- Extract year from founded at string
SELECT 
	id,
	founded_at_clean,
	LEFT(founded_at_clean, 4) year_founded,
	-- Convert to INT 
	CAST(LEFT(founded_at_clean, 4) AS INT) year_founded,
	-- Extract date from founded at string
	CAST(RIGHT(founded_at_clean, 2) AS INT) date_founded,
	-- Extract month from founded at string 
	CAST(SUBSTRING(founded_at_clean, 6, 2) AS INT) date_founded	
FROM companies;

--Trim -- 
--To Clean whitespace / character
SELECT
	 '				Dana			'
	TRIM(LEADING FROM '				Dana			')
	TRIM(TRAILING FROM '				Dana			')
	TRIM(BOTH FROM '				Dana			')
...
SELECT 
	...
	-- Trim Trailing/right from the output 
	TRIM(TRAILING FROM SUBSTRING(permalink, 10, 100)) AS name_trimmed
FROM companies;
-- Extract time information from timestamp type using LEF, RIGHT, & SUBSTRING
SELECT 
	id,
	start_time,
	...
	-- Convert to INT
	LEFT(start_time::TEXT,4)::INT year_rented,
	--Extract minute from start time
	SUBSTRING(start_time::TEXT, 15, 2)::INT year_rented

FROM bikeshare;

--Trim--
-- To Clean whitesspaces/character
...
SELECT
	...
	TRIM(Leading 'w0' FROM UPPER (bike_number)) AS bike_number_trimmed
FROM bikeshare;
...
SELECT
	...
	UPPER(bike_number) bike_number_uppered,
	TRIM(Leading 'W0' FROM UPPER (bike_number)) AS bike_number_trimmed
FROM bikeshare;
...
FROM bikeshare
-- Add condition to filter bike number that have 1 in it
...
SELECT DISTINCT country_code FROM companies;
-- Asia:SGP, IND, CHN,  
-- Europe:ARE , BEL , FRA , CZE , ROM, DNK, ISR,SWE,DEU
-- America:ARG,USA,CAN,BRA
-- Australia: AUS
-- Hell: ISR
SELECT
	id,
	...
	CASE
		...
		WHEN country_code IN ('ARE' , 'BEL' , 'FRA' , 'CZE' , 'ROM', 'DNK', 'ISR','SWE','DEU') THEN 'Europe'
		WHEN country_code IN ('ARG','USA','CAN','BRA') THEN 'America'
...
-----------------------------------
--1.Ambil daftar star_station beserta jumlah perjalanannya masing-masing, 
...
--2. Ambil dftar perusahaan dengan pendanaan terbesr di setiap wilayah
-- yang total pendanaannya lebih dari USD 1000000 dan kapan didanai jelas
SELECT 
